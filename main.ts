...
export default class Prozen extends Plugin {
	...
	fullscreenMode() {
		...
		if (!this.isActive) {
			if (!this.settings.fakeFullScreen) {
	...
	addStyles(leaf: WorkspaceLeaf) {
		...
		if (this.settings.fakeFullScreen) {
			document.body.classList.add("zenpro-fake-fullscreen");
		}
	...
	removeStyles(leaf: WorkspaceLeaf) {
		...
		document.body.classList.remove("zenpro-fake-fullscreen");
...
class ProzenSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Determines how close to the screen's center vignetting spreads from both sides of the screen, as linear gradients."
			)
			.addSlider((slider) =>
				slider
					.setLimits(5, 50, 5)
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Determines how close to the screen's center vignetting spreads from borders of the screen, as a radial gradient."
			)
			.addSlider((slider) =>
				slider
					.setLimits(5, 100, 5)
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Show the scrollbar in Zen mode. If it is hidden, scrolling is still available with mousewheel, arrows, touchpad, etc."
			)
			.addToggle((toggle) =>
				toggle.setValue(this.plugin.settings.showScroll).onChange(async (value) => {
		...
		new Setting(containerEl)
			...
			.addToggle((toggle) =>
				toggle.setValue(this.plugin.settings.showGraphControls).onChange(async (value) => {
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Center text content in Zen mode, even if in regular view it takes all of the screen's width (ignore 'Editor -> Readable line length' being off in Zen mode)"
			)
			.addToggle((toggle) =>
				toggle.setValue(this.plugin.settings.forceReadable).onChange(async (value) => {
		...
		new Setting(containerEl)
			.setName("Fake fullscreen")
			.setDesc("CSS styles only. Manual maximizing/pressing F11 may be required. Works with vim-mode.")
			.addToggle((toggle) =>
				toggle.setValue(this.plugin.settings.fakeFullScreen).onChange(async (value) => {
					this.plugin.settings.fakeFullScreen = value;
					await this.plugin.saveSettings();
				})
			);
	}
